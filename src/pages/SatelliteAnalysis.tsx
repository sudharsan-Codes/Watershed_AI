...
export default function SatelliteAnalysis() {
  ...
  if (!analysis) {
    return <div className="p-6 text-sm text-gis-text-muted">No satellite analysis available for this watershed.</div>;
  }

  return (
    <div className="p-5 h-full flex flex-col bg-gis-bg text-gis-text">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-bold text-gis-text">Satellite Analysis (Prototype)</h2>
      ...
      <div className="relative flex-1 min-h-[360px]">
        ...
        <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-gis-surface/95 backdrop-blur-md border border-gis-border rounded-lg shadow-xl px-6 py-3 flex items-center gap-6">
          <Stat label="Demo NDVI Before" value={analysis.before.toFixed(2)} />
          <Stat label="Demo NDVI After" value={analysis.after.toFixed(2)} />
      ...
      <div className="mt-8 flex items-start justify-between flex-wrap gap-4">
        <div className="flex items-center gap-4 flex-wrap text-[11px] text-gis-text-muted">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-gis-text hover:text-white border border-gis-border hover:border-brand-500 rounded-md px-3 py-1.5 bg-gis-card hover:bg-gis-surface transition-colors shadow-sm"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-gis-text hover:text-white border border-gis-border hover:border-brand-500 rounded-md px-3 py-1.5 bg-gis-card hover:bg-gis-surface transition-colors shadow-sm"
          >
            <GitCompareArrows size={12} className="text-brand-400" />
            View Change Detection
          </button>
        </div>
      </div>

      <p className="text-[11px] text-gis-text-dim mt-3">
...
function LabeledSelect({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center gap-1.5 text-xs border border-gis-border rounded-md px-2.5 py-1.5 bg-gis-card text-gis-text">
      <span className="text-gis-text-dim font-medium">{label}:</span>
      <span className="font-semibold text-gis-text">{value}</span>
    </div>
  );
}
...
function DateField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center gap-1.5 text-xs border border-gis-border rounded-md px-2.5 py-1.5 bg-gis-card text-gis-text">
      <span className="text-gis-text-dim font-medium">{label}:</span>
      <span className="font-semibold text-gis-text">{formatted}</span>
    </div>
  );
}
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "emerald" }) {
  return (
    <div className="text-center">
      <div className="text-[10px] uppercase font-bold tracking-wider text-gis-text-dim">{label}</div>
      <div className={["text-base font-bold", tone === "emerald" ? "text-emerald-400" : "text-gis-text"].join(" ")}>
        {value}
      </div>
    </div>
  );
...
function MetaItem({ label, value }: { label: string; value: string }) {
  return (
    <span>
      <span className="text-gis-text-dim font-medium">{label}:</span>{" "}
      <span className="font-semibold text-gis-text-muted">{value}</span>
    </span>
  );
}
